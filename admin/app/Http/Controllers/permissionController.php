...
class permissionController extends Controller
{
    ...
    public function list()
    {
        ...
        $paginate = $permissionModel->getPagination()->first(); 
        $list_permission = DB::table('tbl_permission')->paginate($paginate->pagination_limitDeaful); 
        $check = $list_permission->hasMorePages() ? 1 : 0;
        $i = 1;
        $category=$paginate->pagination_category;
        $nameElement=$paginate->pagination_nameElement;
        return view('include.main.page.system.permission.lits', compact('list_permission', 'i', 'check','category','nameElement'));
    }
    public function add()
    {
        $permissionModel = new permissionModel();
