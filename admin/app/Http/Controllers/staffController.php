...
class staffController extends Controller
{
    public function list(){
        ...
        $paginate = $staffModel->getPagination()->first(); 
        $list_staff = $staffModel->paginate($paginate->pagination_limitDeaful); 
        $check = $list_staff->hasMorePages() ? 1 : 0;
        $i = 1;
        $category=$paginate->pagination_category;
        $nameElement=$paginate->pagination_nameElement;
        return view('include.main.page.system.staff.lits', compact('list_staff', 'i', 'check','category','nameElement'));
    }
public function add(){
    $staffModel = new staffModel();
...
public function deatil($staff_id){
    ...
    $item_staff =$staffModel->getDetail($staff_id);
    $listMyPemissionId = $staffModel->getMyListTable($staff_id)->get();
    return view('include.main.page.system.staff.detail',compact('item_staff','listMyPemissionId'));
}
